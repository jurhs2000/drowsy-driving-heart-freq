...
participant = int(input('¿Qué conjunto de datos de participante desea utilizar?: '))

# Cargar el dataset
df = pd.read_csv(f'data/processed{processed_no}/data/all_hr_sleep_p{participant:02d}.csv')

logId_unique = df['logId'].unique()
...
i = 1
for logId in logId_unique:
    print(f'logId {logId} ({i}/{len(logId_unique)})')
    ...
    # Título y etiquetas con tamaño de fuente ajustado
